...
public class PositionSync : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        FlagDrop.onValueChanged.AddListener(v =>
        {
            ...
            targetPos = m_SyncRole.localPosition;
    ...
    IEnumerator DelaySend() 
    {
        ...
        if(_lantency <= 0 || _lantency < minLatency)
        {
            ...
            _lantency = _lantency < 0 ? 0 : _lantency;
        }

        m_LastLentacy = _lantency;
    ...
    void OnUpdateNetPosition(PositionData positionData)
    {
        ...
        else
        {
            ...
            { 
                ...
                smoothTime = Mathf.Min(latency, m_MaxLatency) + m_PosSyncInterval;
    ...
    void SyncPositionSmooth()
    {
        ...
        var delta = targetPos - m_SyncRole.localPosition;
        angle.z = Mathf.Rad2Deg * (Mathf.Atan2(delta.y, delta.x) - Mathf.PI * 0.5f);
        if(rate >= 1)
        {
            angle.z = curPosData.angle;
        }
        m_SyncRole.localEulerAngles = angle;
        bSyncFinished = nowTime - recvTime >= smoothTime;
    }
